...
def _unsafe_httpdo(url, method='GET', data=None, timeout=None):
    ...
    try:
        ...
        if isinstance(r.value, dict) and r.value.get("error"):
            ...
            for errCls in (
                    WDAInvalidSessionIdError, WDAPossiblyCrashedError, WDAKeyboardNotPresentError, WDAUnknownError,
...
def _start_wda_xctest(udid: str, port: int, wda_bundle_id=None, mjpeg_server_port=None) -> bool:
    ...
    if port:
        # args.extend(['--port', str(port)])  # not support, use this while for wdaproxy
        args.extend(['-e', f'USE_PORT:{port}'])
    if mjpeg_server_port:
        args.extend(['-e', f'MJPEG_SERVER_PORT:{mjpeg_server_port}'])
...
class BaseClient(object):
    ...
    def press_duration(self, name: str, duration: float):
        ...
        return self._session_http.post("/wda/performIoHidEvent",
                                       {"page": 0x0C, "usage": hid_usage, "duration": duration})
...
class __FBElement(object):
    ...
    def get_attribute(self, attribure):
        """ qiaoliu: get attribure value"""
        if attribure in ["name"]:
            attribure = "text"
        elif attribure == "rect":
            return json.dumps(self._prop('rect'))
        return self.info.get(attribure)

    @property
    def location(self):
        """ qiaoliu: get element obj location dict"""
        value = self._prop('rect')
        return value

    @property
    def size(self):
        """ qiaoliu: get element obj size dict"""
        value = self._prop('rect')
        return value

class Element(__FBElement):
    """ inherit the source Element of facebook-wda"""
...
class USBClient(Client):
    ...
    def __init__(self, udid: str = "", port: int = 8100, wda_bundle_id=None, mjpeg_server_port=None):
        if not udid:
            usbmux = Usbmux()
        ...
        _start_wda_xctest(udid, port, wda_bundle_id, mjpeg_server_port)
